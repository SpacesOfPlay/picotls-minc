// HTTPS GET with ECDSA-P256 cert verify by SPKI pin.
//
// Defaults to example.com with a pin captured at publish time.
// Override with TLS_HOST / TLS_PORT / TLS_SNI / TLS_PATH /
// TLS_PIN_SPKI_SHA256 (64 hex chars).
//
// Compute a fresh pin for any host:
//   openssl s_client -connect HOST:443 </dev/null \
//     | openssl x509 -pubkey -noout \
//     | openssl pkey -pubin -outform der \
//     | openssl dgst -sha256
//
// Servers rotate certs every few months. If the run fails with
// code -5 (handshake), the baked-in pin is stale — pin afresh.
...
i32 main() {
    ...
    vc.super.cb = ecdsa_p256_pinned_verify_cert_cb;
    vc.super.algos = &ecdsa_p256_pl_verify_algos[0];
    for u64 i = 0; i < 32; i++ { vc.pinned_spki_sha256[i] = pin_bytes[i]; }

    printf("GET https://%s:%d%s  (SNI=%s, SPKI-pinned ECDSA-P256)\n",
    ...
    if n < 0 {
        ...
        if n == -5 {
            ...
            printf("  baked-in SPKI pin may be stale — re-pin (see top of file).\n");
        }
        return 1;
    }
